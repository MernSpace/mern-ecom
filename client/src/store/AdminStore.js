...
const AdminStore = create((set) => ({
    ...
    AdminRegisterFormOnChange: (name, value) => {
        set((state) => ({
            AdminRegisterFormData: {
                ...state.AdminRegisterFormData,
                [name]: value
            }
        }))
    },
    AdminRegisterRequest: async (name, email, password, phone) => {
        set({ isFormSubmit: true })
        let res = await axios.post(`/api/v1/register-admin`, {
            name: name,
            email: email,
            password: password,
            phone: phone,
    ...
    AdminLoginRequest: async (email, password) => {
        set({ isFormSubmit: true })
        let res = await axios.post(`/api/v1/admin-login`, {
            email: email,
            password: password
        });
        setEmail(email);
